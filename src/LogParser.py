...
target_type = ['read', 'write', 'readv', 'writev', 'execve', 'fork', 'clone', 'sendto', 'recvfrom', 'recvmsg',
               'sendmsg', 'accept', 'fcntl', 'rename', 'renameat2']
event_list = []
        ...
        if syscall_dir == '>':
            ...
            if len(couple) == 2:
                ...
                if syscall_type in ['execve']:
                    ...
                    event_list.append([event_num, source, destination, syscall_type, size, start_time, end_time])
                    event_num += 1
                    event_list.append([event_num, filename, destination, syscall_type, size, start_time, end_time])
                    # event_num += 1
                ...
                if syscall_type in ['accept']:
                    ...
                    event_num += 1
                    event_list.append([event_num, process, network, syscall_type, size, start_time, end_time])
                    event_num += 1
                    event_list.append([event_num, network, process, syscall_type, size, start_time, end_time])
                    continue
                ...
                if syscall_type in ['rename', 'renameat2']:
                    ...
                    end_time = couple[1][TIME]
                    event_num += 1
                    event_list.append([event_num, old_path, process, syscall_type, size, start_time, end_time])
                    event_num += 1
                    event_list.append([event_num, process, new_path, syscall_type, size, start_time, end_time])
                    continue
